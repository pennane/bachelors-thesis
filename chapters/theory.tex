...
Puhtaista funktioista hyötyy siten, että niitä voi käyttää uudelleen ja uudelleen kontekstista riippummatta. Puhtailla funktioilla voi rakentaa järjestelmäriippumattomia kirjastoja.

Puhtaiden funktioiden perusta on matematiikassa. Jos matemaattinen funktio $f(x) = x+2$ esimerkiksi ei aina palauttaisi samalle $x$:n arvolle samaa palautusarvoa, mitä hyötyä funktiosta edes olisi? Toivottavasti $2 + 2$ tulee aina olemaan $4$.
...
Kategoriateoria on koettu ohjelmoijille erityisen mieluisaksi matematiikan haaraksi. Olisi algebra tai kalkyyli ohjelmoillalle jo ajatuksena hirveää, kategoriateorian on koettu olevan ohjelmoijille erityisen mieluisaa laajasta teoreettisuudesta huolimatta. \cite[9]{milewski2017category}.
...
Kategoriateoriaa ei käsitellä tässä opinnäytetyössä paljoa, sillä kirjoittaja ei tunne taitavansa sitä tasolla, että voisi tiivistää tärkeimmät asiat oikealle tasolle. Kategoriateoria on kuitenkin niin kytköksissä funktionaaliseen ohjelmointiin, ettei sitä voinut jättää kokonaan maitsematta. Kategoriateorialle on kaivattu ohjelmoille käytännönläheisempää tapaa lähestyä sitä \cite{holvikari2021category}. Toisaalta on myös huomattu, että ohjelmoitaessa funktionaalista ohjelmointia puhtaasti, kategoriateorian oppii automaattisesti työn ohessa ilman tiukkaa teorian opiskelua \cite{holvikari2021category}.
...
JavaScript ei ole funktionaalinen ohjelmointikieli vaikka se sitä tukeekin \cite{is_reduce_bad}. TypeScriptin tyyppijärjestelmällä funktionaalinen ohjelmointi on jo mieluisampaa \cite{holvikari2021category}.


\subsection{Kielen sopivuus funktionaaliseen ohjemointiin}

Vaikka TypeScript ei ole pohjimmiltaan funktionaalinen ohjelmointikieli, on sillä ominaisuuksia joilla funktionaalisesta ohjelmoinnista tulee verrattaen helppoa.

TypeScriptissä on käytännössä kolme eri tapaa määrittää funktioita (Esimerkkikoodi \ref{code:javascript_function_types}). Nuolifunktioilla voidaan harjoittaa funktionaaliselle ohjelmoinnille tyypillistä argumenttien yksi kerrallaan funktiolle antamista.
...
Kokemusten perusteella syntaksierot ovat todella hidastava tekijä. Ohjelmoijat olettavat ohjelmoivansa TypeScript projektissa TypeScriptiä. Tai kieliriippumattomasti sanottuna, ohjelmoijat olettavat ohjelmoivansa sitä ohjelmointikieltä mitä ohjelmoidaan.
...
Selaimille on myös parempi olla rikkomatta näitä vanhoja sivuja. Jos vierailee lääkärin vastaanoton verkkosivulla selvittääksesi aukioloajat, mikä selain on paras: se, joka näyttää vastaanoton aukioloajat, vai se, joka ei näytä mitään, koska on ottanut uuden JavaScript-version käyttöön? \cite{against_self_closing_tags,proposal-joint-iteration}

Noin kymmenen vuotta sitten oli suuri keskustelu siitä, tulisiko JavaScriptin Promise-tietorakenteen noudattaa funktionaalisen ohjelmoinnin monadin rakennetta. Tälle oli suuret perusteet, sillä rakenteen noudattimen olisi vaatinut vain pieniä muutoksia ehdotettuun tietorakenteen toteutukseen \cite{promises-spec-94}. Perustelut monadin puolesta eivät kuitenkaan menneet lävitse muunmuassa väärinymmärrysten takia.

Monadin läsnäolo kielessä olisi mahdollistanut kielen viennin funktionaalisempaan suuntaan. Ja koska ohjelmointikielelle on tärkeää olla johdonmukainen, on erittäin epätodennäköistä, että kieli tulee saamaan enää koskaan toista mahdollisuutta tuoda monadia osaksi ominaisuuksia.
en lähemmäs funktionaalisen ohjelmoinnin ihanteita \cite{proposal-joint-iteration,prototype_library_trends}.

Promise-tietorakenne on nykypäivänä erittäin vahvassa roolissa ohjelmoitaessa JavaScriptiä. Jos maailma olisi seurannut vaihtoehtoista historiaa, kieli saattaisi tänäpäivänä tukea monia vastaavanlaisia ja standardoidusti toimivia tietorakenteita \cite{promises-spec-94}.
\section{Ramda.js}
