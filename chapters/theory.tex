...
Kategoriateoriaan ei perehdytä työssä paljoa Monadi-rakennetta lukuunottamatta. Syynä on, ettei ole käytännönläheistä tuoda tavattoman teoreettisia käsitteitä työhön, jossa tavoitteena on etsiä konkretiaa.
...
\begin{itemize}
    \item Monadi on asia, jolla on \mintinline{javascript}|flatMap|-metodi \cite{stackoverflow_flatmap_monad}.
...
Monadi on suunnittelumalli, joka tarjoaa tavan käsitellä operaatioiden ketjuttamista (\glsdisp{composed_function}{yhdistetyt funktiot}) ilman, että täytyy huolehtia suoraan siitä, miten data kulkee funktiosta toiseen. Monadi voi kääriä jonkin arvon tiettyyn \textquote*{laatikkoon}, tai kontekstiin, ja pitää sisällään abstraktion, joka mahdollistaa lisätoimenpiteiden suorittamisen, vaikka sisällä oleva arvo voi vaihdella.
...
Monadilla on oltava kaksi toimintoa: \mintinline{javascript}|unit| ja \mintinline{javascript}|bind|. Nämä on johdettu kategoriateorian monadin määritteestä. \citep{bartosz_category_for_progamers_10}

\mintinline{python3}|of|-toiminnolla kääritään arvoja monadiin. \mintinline{python3}|of|-toiminnon voidaan ajatella olevan monadin konstruktori.

\mintinline{javascript}|bind|-toiminnolla muutetaan monadiin käärittyä arvoa antamalla toiminnolle funktio, joka palauttaa toisen saman tyypin monadin. \mintinline{javascript}|bind|-toiminto on käytännössä sama kuin tunnetumpi \mintinline{javascript}|map|-funktio. Erona on se, että \mintinline{javascript}|map| palauttaa arvon, joka on kääritty uuteen monadiin, mutta \mintinline{javascript}|bind| mahdollistaa arvojen ketjuttamisen ja säilyttää monadin rakenteen, jolloin saadaan lisätoimintoja, kuten virheiden käsittely tai asynkroninen suoritus.

Jos monadille on implementoitu \mintinline{javascript}|map| metodi, se ei välttämättä muokkaa monadin rakennetta, kun taas \mintinline{javascript}|bind| yhdistää useita operaatioita, jotka riippuvat edellisen operaation tuloksista. Tämä tekee \mintinline{javascript}|bind|-toiminnosta erityisen voimakkaan työkalun monadien käytössä, sillä sen avulla voidaan rakentaa monimutkaisempia operaatioita hallitusti ja ennakoitavasti.
...
Array käyttää \mintinline{javascript}|flatMap|-metodia bindina ja staattista \mintinline{javascript}|Array.of|-metodia unitina.

\textit{bind}: \mintinline{javascript}|Array.prototype.flatMap(f)|

\textit{unit}: \mintinline{javascript}|Array.of(x)|
...
\textit{bind}: \mintinline{javascript}|Promise.prototype.then(f)|

\textit{unit}: \mintinline{javascript}|Promise.resolve(x)| ja \mintinline{javascript}|Promise.reject(x)|
...
Ilman kommentteja monadisen ketjutuksen näppäryyden huomaa vielä selkeämmin.
...
Tässä insinöörityössä tarkempia pragmaattisia käytäntöjä pyritään löytämään ja arvioidaan neljännessä osassa, ja siitä eteenpäin.
