...
\end{minted}
    \caption{Funktio, muuttaa listan käyttäjiä sellaisiksi, että niitä voidaan käyttää tuloslistoilla. Funktio on yhdistetty monesta funktiosta käyttämällä Ramda.js kirjaston funktioita}
...
.sort((userA, userB) => userB.score - userA.score)
    \end{minted}
    \caption{Toiminnaltaan sama funktio, mutta käytettynä on vain JavaScriptin sisäänrakennettuja palikoita}
...
Funktioilla on kiva kikkailla, mutta kikkailu ei nopeasti toteutettuna kanna kauas.
...
Esimerkkinä funktio, joka ottaa sisään listan avain-arvo-pareja, ja luo niistä olion. Näytetään kaksi (funktionaalista) tapaa toteuttaa funktio.
...
Funktionaalinen ohjelmointi mahdollistaa deklaratiivisen, itsensä selittävän ohjelmakoodin kirjoittamisen. On vaikea nimetä funktioita, tai muita ohjelmiston palikoita oikein, jos koko ongelma on mallinnettu väärin. Tässä osassa pyritään käymään miten ongelmia voi mallintaa pragmaattisesti. Ohjelman ympäristön malli rakennetaan pala palalta kuvitteelliselle alustalle.
...
Pääasia on, että tyyppi kuvaa listaa, joka ei voi olla tyhjä. Tyypillä on kenttä \mintinline{javascript}|first|, jonka arvon on oltava olemassa. Ja kenttä \mintinline{javascript}|rest|, jossa säilytetään listan loput alkiot. Tuon kentän lista taas on tavallinen \mintinline{javascript}|Array|. Ja se saa olla tyhjä. (Esimerkkikoodi \ref{code:ts_non_empty_quiz}).
