...
\usepackage{siunitx}  % Load siunitx package for SI units
...
\tcbuselibrary{listingsutf8} % For including monospace font settings

\newcommand{\key}[1]{%
	\begin{tcolorbox}[
			colback=gray!20, % Light gray background
			colframe=gray!20, % Same as background, effectively invisible
			...
			boxsep=0pt, % No separation between text and frame, padding only
			height=12pt,
			valign=center,
			nobeforeafter, % No additional space before and after the box
			tcbox raise base, % Aligns the box with the baseline of the surrounding text
			hbox % Ensures the box is only as wide as its contents
		]
		\texttt{\small #1} % Smaller monospaced font for the key text
	\end{tcolorbox}
}
...
%IMAGE - FIGURE
%the figures should be placed in the "figures" folder
...
\newenvironment{code}
{
	\begin{samepage} % Start keeping content on the same page
		\captionsetup{type=listing}
		}
		{
	\end{samepage} % End keeping content on the same page
}
...
\DeclareFieldFormat*{url}{%
	...
	\ifentrytype{podcast}{Podcast\addperiod\space}{}%
	\textless\url{#1}\textgreater\addperiod
} % you can modify how to url looks here

\DeclareFieldFormat{urldate}{%
	\ifentrytype{video}{\space Katsottu\space#1}{%
		\ifentrytype{podcast}{\space Kuunneltu\space#1}{\space Luettu\space#1}}%
}
...
\ifdefstring{\thesiscopy}{all}{%
	...
	\def\copyfi{\otsikko\ \textcopyright\ \the\year{}, jonka tekijä on \theauthor, on julkaistu {\doclicenseLongNameRef} lisenssillä}
}
\hypersetup{%
	pdfdisplaydoctitle,
